  ...
  const handleSubscriptionChange = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || `Failed to ${action} plan. Please try again.`);
      }
    } catch (err) {
        ...
        <div className="mb-6">
          {isUpgrade ? (
            <div className="text-gray-700">
              <p>You're about to upgrade to the Pro Plan with unlimited notes.</p>
              <p className="mt-4"><span className="font-medium">Pro Plan Benefits:</span></p>
            ...
            <div className="text-gray-700">
              ...
              <p className="mt-4"><span className="font-medium text-yellow-600">Warning:</span></p>
              <ul className="list-disc pl-5 mt-2 space-y-1 text-yellow-600">
                <li>You will be limited to 3 notes maximum</li>
                <li>If you currently have more than 3 notes, you'll need to delete some before using the app</li>
              </ul>
            </div>
          )}
        </div>
